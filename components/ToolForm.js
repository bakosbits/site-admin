        ...
        <form method="POST" className="space-y-6 bg-cardDark p-8 rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <div className="mt-2 grid grid-cols-2 md:grid-cols-3 gap-2 bg-gray-700 p-4 rounded-md">
                        {pricingOptions.map((option) => (
                            <div key={option} className="flex items-center">
                                <input
                                    ...
                                    className="h-4 w-4 text-accentGreen bg-gray-800 border-gray-600 rounded focus:ring-accentGreen"
                                />
            ...
            <div>
                ...
                <div className="mt-2 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-h-60 overflow-y-auto bg-gray-700 p-4 rounded-md">
                    {categories.map((cat) => (
            ...
            <div className="flex space-x-6">
                <div className="flex items-center">
                    <input type="checkbox" name="Active" id="Active" className="h-4 w-4 text-accentGreen bg-gray-700 border-gray-600 rounded focus:ring-accentGreen" defaultChecked={tool?.Active || false} />
                    <label htmlFor="Active" className="ml-2 block text-sm text-gray-300">Active</label>
                </div>
                <div className="flex items-center">
                    <input type="checkbox" name="Featured" id="Featured" className="h-4 w-4 text-accentGreen bg-gray-700 border-gray-600 rounded focus:ring-accentGreen" defaultChecked={tool?.Featured || false} />
                    <label htmlFor="Featured" className="ml-2 block text-sm text-gray-300">Featured</label>
                </div>
            </div>

            <div className="flex justify-end space-x-4 pt-4">
                <Link href="/tools" className="bg-gray-600 text-white font-bold py-2 px-4 rounded hover:bg-gray-500 transition-colors">Cancel</Link>
                <button type="submit" className="bg-accentGreen text-backgroundDark font-bold py-2 px-4 rounded hover:bg-headingWhite transition-colors">{isNew ? 'Create Tool' : 'Update Tool'}</button>
            </div>
        </form>
    );
}