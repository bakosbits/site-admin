    ...
    return (
        <form method="POST" className="space-y-6 bg-cardDark p-8 rounded-lg">
            ...
            <div className="flex items-center">
                <input type="checkbox" name="Published" id="Published" className="h-4 w-4 text-accentGreen bg-gray-700 border-gray-600 rounded focus:ring-accentGreen" defaultChecked={article?.Published || false} />
                <label htmlFor="Published" className="ml-2 block text-sm text-gray-300">Published</label>
            </div>

            <div className="flex justify-end space-x-4 pt-4">
                <Link href="/articles" className="bg-gray-600 text-white font-bold py-2 px-4 rounded hover:bg-gray-500 transition-colors">Cancel</Link>
                <button type="submit" className="bg-accentGreen text-backgroundDark font-bold py-2 px-4 rounded hover:bg-headingWhite transition-colors">{isNew ? 'Create Article' : 'Update Article'}</button>
            </div>
        </form>
    );
}